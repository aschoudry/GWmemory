...
wave = load('/home/ashok/gravitational_wave_memory_project/tests/testfile.txt');
timeNR = wave(:,1);
psi_plus = wave(:,2);
psi_cross = wave(:,3);
timeNR=timeNR(3500:21550);
psi_plus=psi_plus(3500:21550);
psi_cross=psi_cross(3500:21550);
...
save hdotNR.dat A -ASCII -double
...
plot(timeNR,h_cross)
A=[timeNR h_plus h_cross];
save hNR.dat A -ASCII -double

h_plus_mem =cumsum(abs(hdot_plus).*abs(hdot_plus))*(timeNR(2)-timeNR(1))/(192*pi);
