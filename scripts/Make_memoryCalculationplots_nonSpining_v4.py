...
#data location
file_location ='../data/NonSpinning_differentMassRatio/Memory_data/'


mass_ratio_vec = [2.0, 3.0, 4.0, 7.0]
filename_vec=['q2', 'q3', 'q4', 'q7']
line_thickness_vec = [1,1.5,2.0,3.0]
...
for filename in filename_vec:
	...
	plt.plot(time_tot, hmem_tot, label='q = '+str(mass_ratio_vec[i]),linewidth=line_thickness_vec[i])
	i+=1
...
plt.plot(time_PN_original+ 555, hp_mem_PN_original, 'r--',label='PN')
plt.grid()
...
for filename in filename_vec:
	...
	ax2.plot(time_tot, hmem_tot,linewidth=line_thickness_vec[i])
	i+=1
...
ax2.plot(time_PN_original+ 555, hp_mem_PN_original, 'r--')

plt.xlim(-100, 50)
plt.ylim(0.0045, 0.027)
plt.xticks([-50 ,0, 50], fontsize=10)
plt.yticks([0.005,0.015, 0.025], fontsize=10)
plt.grid()
plt.savefig('../plots/MemoryPlot_nonSpining/'+filename+'.pdf')
plt.show()
