...
numer_of_observation_days=5.0*365.0	
...
for i in range(len(Spin_array)):
	for j in range(len(Mass_array)):
		Memory_growth_M_vs_Spin[i][j]  = 4.6*(10**(Mass_array[j]-23))*Memory_growth_in_two_weeks(Mass_array[j], Spin_array[i], numer_of_observation_days)

fig, ax = plt.subplots()
im = plt.imshow(Memory_growth_M_vs_Spin, interpolation='bilinear', cmap=cm.RdYlGn, origin='lower', extent=[-0.94, 0.99, 8, 10.0], norm=LogNorm(vmin=9.34e-17, vmax=3.33e-14))
plt.xlabel('Spin')
plt.ylabel('Log{(M)')
...
def compute_rms_reseduals(log_SolarMass, Spin, numer_of_observation_days): 
	...
	timeNR_numer_of_observation_days=time_numer_of_observation_days*4.6*pow(10.0, -6)*pow(10, log_SolarMass)
...
for i in range(len(Spin_array)):
	for j in range(len(Mass_array)):
		Reseduals_M_vs_Spin[i][j]  = compute_rms_reseduals(Mass_array[i], Spin_array[j], numer_of_observation_days)

fig, ax = plt.subplots()
im = plt.imshow(Reseduals_M_vs_Spin , interpolation='bilinear', cmap=cm.RdYlGn, origin='lower', extent=[-0.94, 0.99, 8, 10.0], norm=LogNorm(vmin=5.70e-12, vmax=7.4e-09))
plt.xlabel('Spin')
plt.ylabel('Log{(M)')
plt.colorbar()
fig.tight_layout()
