...
#function that computes the Postnewtonian memory for 10 years for 10^10 Msun Binary
def Postnewtonian_MemoryGrowth(dt, chiZa, chiZs):
    t_iPN=-3212.0
    t_fPN=-770.0
...
for i in Spin_array:
	for j in Mass_array:
		...
		hmem = hmem+hp_mem_PN[-len(timeNR)]
		hmem_two_weeks=hmem_two_weeks+hp_mem_PN[-len(timeNR)]

		ax1.plot(time_PN, hp_mem_PN, 'r:')		
		ax1.plot(timeNR, hmem,'k--')

		ax1.plot(time_two_weeks, hmem_two_weeks, label='$M=10 ^{'+ str(round(j,1))+'}M_\odot\t\, \, S ='+str(round(i, 2))+'$')
		k+=1		
	l+=1
...
for i in Spin_array:
	for j in Mass_array:
		timeNR_two_weeks = compute_rms_reseduals(j, i, numer_of_observation_days)[0]		
		res_two_weeks = compute_rms_reseduals(j, i, numer_of_observation_days)[1]
		res_quad_fit_two_weeks = compute_rms_reseduals(j, i, numer_of_observation_days)[2]
		res_quadSubtract_two_weeks = compute_rms_reseduals(j, i, numer_of_observation_days)[3]
