...
def H1p5(theta, eta, chiZa, chiZs):
	H1p5 = (np.sin(theta)**2)*(chiZa/1280.0 + 153.0*chiZs/320.0 + (27.0/56.0)*chiZa*np.sqrt(1.0-4.0*eta) + (chiZs/1152.0)*np.sqrt(1.0 - 4.0*eta) - (9.0*chiZa/2240)*np.sqrt(1.0-eta**2)-\
		(chiZs/11520.0)*(1.0-eta**2)**(3.0/2.0) + (7.0*chiZa/11520.0)*np.cos(theta)**2 + (9.0/320.0)*chiZs*np.cos(theta)**2 +\
		 (9.0/320.0)*chiZa*np.sqrt(1.0-eta**2)*np.cos(theta)**2+\
		(7.0/11520.0)*chiZs*((1.0-eta**2)**(3.0/2.0))*np.cos(theta)**2 + eta*(-(chiZa/320.0) - (187.0/720.0)*chiZs - (chiZs/288.0)*np.sqrt(1.0- 4*eta)-\
		 (7.0*chiZa/2880.0)*np.cos(theta)**2 - (11.0*chiZs/720.0)*np.cos(theta)**2))	
	return H1p5
...
Spin_vec = [0.2, 0.43, 0.60, 0.8, 0.95]
filename_vec=['0p2', '0p43', '0p600', '0p800', '0p95']
tf_vec=[-300, -40, 0, 0, 0.0]
ti_vec=[-7000, -8000, -4500, -2000, -1300]
idx_cut_vec = [14400, 13000, 16530, 17150, 17950]
...
#	print idx, len(dhp_mem_PN_dt), q
	...
	plt.title('Memory Calculation for Spinning for Sz = '+str(Spin_vec[i]) ,fontsize = 15)
	plt.plot(time_PN, dhp_mem_PN_dt, 'r',label=r'$\frac{dhmem_{PN}}{dt}$')
	plt.plot(timeNR[:NRmax_idx],dhp_mem_NR_dt, 'g',label=r'$\frac{dhmem_{NR}}{dt}$')
	plt.grid()
	plt.xlabel(r'$time$')
	plt.ylabel(r'$dh_{mem}/dt$')
	plt.legend(loc=2)
	fontP.set_size('20.')

	plt.show()

	plt.title('Memory Calculation for NonSpinning for Sz = '+str(Spin_vec[i]) ,fontsize = 15)
	plt.plot(time_tot, hmem_tot, label=r'$hmem_{PN}$')
	plt.plot(timeNR_cut, hmem_tot_cut, 'y--', label=r'$hmem_{NR}$' )
