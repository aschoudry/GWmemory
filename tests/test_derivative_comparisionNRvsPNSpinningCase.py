...
#	print idx, len(dhp_mem_PN_dt), q
	...
	plt.title(r'$\chi_z = $'+str(Spin_vec[i]) ,fontsize = 15)
	plt.plot(time_PN, dhp_mem_PN_dt, 'r',label=r'$\frac{dh^{(mem)}_{PN}}{dt}$')
	plt.plot(timeNR[:NRmax_idx],dhp_mem_NR_dt, 'g',label=r'$\frac{dh^{(mem)}_{NR}}{dt}$')
	plt.grid()
	plt.xlabel(r'$time$')
	plt.ylabel(r'$dh^{(mem)}/dt$')
	plt.xlim(-500, 20.0)
	plt.ylim(0, 0.001)
	plt.legend(loc=2)
#	fontP.set_size('20.')
	plt.savefig("/home/ashok/gravitational_wave_memory_project/plots/DerivativeComparisionNRvsPN"+str(filename)+".pdf")
	plt.show()
